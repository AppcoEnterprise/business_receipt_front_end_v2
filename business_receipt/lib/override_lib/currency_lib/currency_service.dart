...
class CurrencyService {
  ...
  // CurrencyService() : _currencies = currenciesGlobal;

  ///Return list with all currencies
  List<Currency> getAll() {
    return currenciesOptionGlobal;
  }

  ///Returns the first currency that mach the given code.
  Currency? findByCode(String? code) {
    final uppercaseCode = code?.toUpperCase();
    return currenciesOptionGlobal.firstWhereOrNull((currency) => currency.code == uppercaseCode);
  }

  ///Returns the first currency that mach the given name.
  Currency? findByName(String? name) {
    return currenciesOptionGlobal.firstWhereOrNull((currency) => currency.name == name);
  }

  ///Returns the first currency that mach the given number.
  Currency? findByNumber(int? number) {
    return currenciesOptionGlobal.firstWhereOrNull((currency) => currency.number == number);
  }
